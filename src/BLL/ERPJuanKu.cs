...
namespace ZWL.BLL
{
	...
	public class ERPJuanKu
	{
		...
		public ERPJuanKu(int ID)
		{
			...
			strSql.Append("select ID,JuanKuName,JuanKuSerils,SuoShuBuMen,BackInfo,UserName,TimeStr ");
		...
		return DbHelperSQL.GetMaxID("ID", "ERPJuanKu"); 
		}
		...
		public int Add()
		{
			...
			strSql.Append("JuanKuName,JuanKuSerils,SuoShuBuMen,BackInfo,UserName,TimeStr)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,JuanKuName,JuanKuSerils,SuoShuBuMen,BackInfo,UserName,TimeStr ");
			strSql.Append(" FROM ERPJuanKu ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
